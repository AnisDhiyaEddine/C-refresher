...
// $$jbvoron$$ Modif ASR: Aucune prise en compte des priorites
// Fonction qui affiche la file d'attente
void printWaitingProcesses() {
	int k;
	DEBUG ("LISTE DES PROCESSUS EN ATTENTE <processus ID>\n");
	for (k=0;k<MAXPROC;k++) {
		if (Tproc[k].flag == RUN) {
			DEBUG("<%d> ", k);
		}
...
int commut(int s) {
	...
	if (verbose) DEBUG("FIN DU QUANTUM ==> COMMUTATION !\n\n");
	...
	} else if (ssize > Tproc[courant].stack_size) {
...
int election(void) {
	...
	// $$jbvoron$$ Modif ASR pour eviter un glitch sur le premier processus
	static int cpt = 0; 
	unsigned long j;
	int p;
	int maxprio = MINPRIO;
	...
	// $$jbvoron$$ Modif ASR: Ajustement de la priorite
	if ((cpt++ > 0) && (Quantum > 0) && (Tproc[elu].prio > MINPRIO) && (elu >= 0)) {
		DEBUG ("CHANGEMENT DE FILE POUR %d (OLD=%d ;; NEW=%d)\n", elu, Tproc[elu].prio, Tproc[elu].prio-1);
		Tproc[elu].prio--;
	}
	
	// $$jbvoron$$ Modif ASR: Pour afficher plus de details
	printWaitingProcesses();
	
	p = 0;
	j = (i-1)%MAXPROC;

	for(i = 0; i != j; i=(i+1)%MAXPROC) {
	...
	if (verbose) DEBUG ("\n");
	if (verbose) DEBUG ("ELECTION DU PROCESSUS %d\n",p);

	return p;
}	

// Fixer les paramètres de l'ordonnanceur
void SchedParam(int type, int timeslice, int (*elec)(void)) {
...
int CreateProc(function_t f, void *arg, int d) {
	...
	// $$jbvoron$$ Modif ASR: Gestion des files pour les MLRR
	Tproc[i].file = 0;
...
void InitSched(void) {
	...
	for (i=0; i < MAXPROC; i++) {
		...
		// $$jbvoron$$ Modif ASR: Initialisation des processus et placement dans la file 0
		Tproc[i].file = 0;
	}
...
// Recuperer les noms de fonctions
	...
	printf("PID\tREAL-TIME\tCPU-TIME\tWAITING-TIME\n");
	printf("--------------------------------------------------------------------\n");
	for (i = 0; i<MAXPROC; i++) {
